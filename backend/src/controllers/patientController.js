...
const viewHealthFam= async(req,res)=>{
  try {//changed this
    const username=req.user.Username;
    const Patient= await patientModel.findOne({Username:username});
    const famMems= await familyMemberModel.find({$or:[{"Patient":Patient},{"FamilyMem":Patient}]}).populate("Patient").populate("FamilyMem"); 
    //check eno family mem mesh user
    const package = await Promise.all(famMems.map( async(famMember) => {
      const value= await patientModel.findOne({Username:famMember.Patient.Username})
      const subscription =famMember.FamilyMem!=null? await subscriptionModel.findOne({ Patient: famMember.FamilyMem.Username===username?value:famMember.FamilyMem,Status:"Subscribed"}).populate("Patient").populate('FamilyMem').populate('Package'):await subscriptionModel.findOne({ FamilyMem: famMember,Status:"Subscribed"}).populate("Patient").populate('FamilyMem').populate('Package')
...
const payForSubscription= async(req,res)=>{
  try{
    ...
    const patSubscription= await subscriptionModel.findOne({Patient:patient}).populate('Package')
    if(!Package){
  ...
  }).populate("Patient").populate("FamilyMem");   
    var max=0
    for (let i=0;i<patientRelatives.length;i++) {
      const value= patientRelatives[i].Patient.Username===curr_user? await subscriptionModel.findOne({Patient:patientRelatives[i].FamilyMem}).populate('Package').populate("Patient"):await subscriptionModel.findOne({Patient:patientRelatives[i].Patient}).populate('Package').populate("Patient")
      if (value && value.Status === 'Subscribed' && value.Package.Family_Discount > max) {
...
const viewHealthPackage = async (req, res) => {
  try {
    ...
    const subscription = await subscriptionModel.findOne({Patient:patient, Status:"Subscribed"}).populate("Patient").populate("Package")
...
const viewHealthPackagewithstatus = async (req, res) => {
  try {
    ...
    const subscription = await subscriptionModel
      ...
      .populate("Patient")
      .populate( "FamilyMem" ).populate("Package");
...
const viewHealthFamwithstatus= async(req,res)=>{
  try {//changed this
    ...
    const package = await Promise.all(famMems.map( async(famMember) => {
      const value= await patientModel.findOne({Username:famMember.Patient.Username})
      const subscription =famMember.FamilyMem!=null? await subscriptionModel.findOne({ Patient: famMember.FamilyMem.Username===username?value:famMember.FamilyMem}).populate("Patient").populate('FamilyMem').populate('Package'):await subscriptionModel.findOne({ FamilyMem: famMember}).populate("Patient").populate('FamilyMem').populate('Package')
...
// Renewal date m4 subscribes 
  ...
  if(subscription.Status==="Subscribed" && formattedDate===formattedDate4 &&subscription.Package.Name==Package.Name ){    
    ...
    res.status(200).json("Payment for subscription completed successfully!")
    }
...
const GetFamilymembers = async (req, res) => {
  try {
    ...
    const fam = await familyMemberModel.find({Patient:patient}).populate("Patient").populate("FamilyMem");
     console.log(Patient)
...
//     const Package = await  packageModel.findOne({Name:Package});
