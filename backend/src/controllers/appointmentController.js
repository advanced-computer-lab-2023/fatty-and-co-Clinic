...
const getAppointments= async(req,res)=>{
    ...
    res.status(200).send(result)
        ...
        const result= current_type==="Doctor"?await appointmentModel.find({DoctorUsername:current_user,Date:new Date(dateSearch)}) : await appointmentModel.find({PatientUsername:current_user, Date:new Date(dateSearch)})
        res.status(200).send(result)
    }
