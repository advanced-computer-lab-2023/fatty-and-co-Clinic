...
function PatientTable() {
    ...
    const fetchPatient = () => {
        ...
        let url = API_PATHS.viewDoctorPatients + `?DoctorUsername=${doctorUsername}`;
        if (filters.doctorUsername) url = API_PATHS.viewDoctorPatients + `?DoctorUsername=${filters.doctorUsername}`;
        if (filters.patientName) url += `&PatientName=${filters.patientName}`;
        if (filters.upcoming) url = API_PATHS.viewUpcomingAppointments + `?DoctorUsername=${filters.doctorUsername}`;
    ...
    const openInfoModal = async (patientUsername) => {
        try {
            const response = await axios.get(`http://localhost:8000/doctor/viewPatientInfoAndHealthRecords?PatientUsername=${patientUsername}`);
                ...
                <Modal isOpen={isModalOpen} onClose={closeModal}>
                    ...
                    <ModalContent>
                        ...
                        <ModalBody>
                            ...
                            <Button onClick={() => { openInfoModal(selectedPatient.Username) }}>
                            ...
                            <ul style={{ listStyleType: 'decimal' }}>
                                {selectedPatientViewInfo.appointments.map((appointment, index) => (
                                    <li key={index}>
                                        ...
                                        {index < selectedPatientViewInfo.appointments.length && <hr />}
                                    </li>
                                ))}
                            </ul>

                            <Heading as="h5" size="sm">Prescriptions:</Heading>
                            <ul style={{ listStyleType: 'decimal' }}>
                                {selectedPatientViewInfo.prescriptions.map((prescription, i) => (
                                    <li key={i}>
                                        <p>Date: {prescription.Date}</p>
                                        <p>Diagnosis: {prescription.Diagnosis}</p>
                                        <p>Status: {prescription.Status}</p>
                                        <p>Medicine:</p>
                                        <ol style={{ listStyleType: 'lower-alpha', marginLeft: '40px' }}>
                                            {prescription.Medicine.map((medicine, j) => (
                                                <li key={j}>{medicine.Name}</li>
                                            ))}
                                        </ol>
                                        {i < selectedPatientViewInfo.prescriptions.length && <hr />}
                                    </li>
                                ))}
                            </ul>
